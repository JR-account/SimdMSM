...
template<typename T, typename FieldT, multi_exp_method Method,
    typename std::enable_if<(Method == multi_exp_method_pip_avx), int>::type = 0>
T multi_exp_inner(
    ...
    typename std::vector<FieldT>::const_iterator exponents_end)
{
    ...
    int length = bases_end - bases;
    int log2_length = log2(length);
    T result;
    core_init();
    fp_prime_init();
    ep_curve_init();
    ...
    bn_t *k = (bn_t *)malloc(length * sizeof(bn_t));


    for (int i = 0; i < length; i++)
    {
        Fr_to_bn(k[i], exponents[i]);
    }

    const libff::bls12_381_G1* ptr = &(*bases); // 获取指向 bases 迭代器指向元素的指针
    ep_t* P = reinterpret_cast<ep_t*>(const_cast<libff::bls12_381_G1*>(ptr));

    pip_ctx ctx;
    clock_t start, end;
    int best_wsize[] = {9, 10, 11, 11, 13, 14, 15, 15, 15, 15};

    if(log2_length < 10)
    {
        ctx.WBITS = log2_length - 2;
        printf("length = %d\n", length);
        printf("log2_length = %d\n", log2_length);
        printf("WBITS = %d\n", ctx.WBITS);
        ctx.NWINS = ((NBITS + ctx.WBITS - 1) / ctx.WBITS);
        ctx.BUCKETNUM = (1 << ctx.WBITS) - 1;
        ctx.bucket = (Bucket*)malloc(ctx.BUCKETNUM * sizeof(Bucket));
        start = clock();
        Pippenger_old(length, *((ep_t*) &result), k, P, &ctx);
        end = clock();
        printf("Pippenger_old time = %f\n", (double)(end - start) / CLOCKS_PER_SEC);
        ep_norm(*((ep_t*) &result), *((ep_t*) &result));

        free(k);
        free(ctx.bucket);
        return result;
    }
    else if(log2_length < 12)
    {
        ctx.WBITS = log2_length - 3;
    }
    else if(log2_length < 15)
    {
        ctx.WBITS = log2_length - 5;
    }
    else if(log2_length < 25)
    {
        ctx.WBITS = best_wsize[log2_length - 15];
    }
    else
    {
    ...
    pippenger_ifma(k, P, length, *((ep_t*) &result), M, &ctx);
    ep_norm(*((ep_t*) &result), *((ep_t*) &result));
...
template<typename T, typename FieldT, multi_exp_method Method,
    typename std::enable_if<(Method == multi_exp_method_pair_avx), int>::type = 0>
T multi_exp_inner(
    typename std::vector<T>::const_iterator bases,
    typename std::vector<T>::const_iterator bases_end,
    typename std::vector<FieldT>::const_iterator exponents,
    typename std::vector<FieldT>::const_iterator exponents_end)
{
    ...
    const libsnark::knowledge_commitment<libff::bls12_381_G2, bls12_381_G1>* ptr = &(*bases); // 获取指向 bases 迭代器指向元素的指针
    pair* P = reinterpret_cast<pair*>(const_cast<libsnark::knowledge_commitment<libff::bls12_381_G2, libff::bls12_381_G1>*>(ptr));

    pair_msm_ctx ctx;
    int best_wsize[] = {9, 10, 10, 10, 11, 12, 15, 15, 15, 15};
    ...
    if(log2_length < 8)
    {
        ...
        Pippenger_pair_old(length, *((pair*) &result), k, P, &ctx);

        ep_norm((*((pair*) &result))->g1, (*((pair*) &result))->g1);
        ep2_norm((*((pair*) &result))->g2, (*((pair*) &result))->g2);
        free(k);
        free(ctx.bucket);
        return result;
    }
    ...
    pippenger_pair_ifma(k, P, length, *((pair*) &result), M, &ctx);
    ep_norm((*((pair*) &result))->g1, (*((pair*) &result))->g1);
    ep2_norm((*((pair*) &result))->g2, (*((pair*) &result))->g2);
...
template<typename T, typename FieldT, multi_exp_method Method,
    typename std::enable_if<(Method == multi_exp_method_pip_avx_threads), int>::type = 0>
T multi_exp_inner(
    typename std::vector<T>::const_iterator bases,
    typename std::vector<T>::const_iterator bases_end,
    typename std::vector<FieldT>::const_iterator exponents,
    typename std::vector<FieldT>::const_iterator exponents_end)
{
    ...
    {
        wsize = best_wsize[log2_length - 15];
    }
    else
    {
        wsize = log2_length - 9;
    ...
    pip_threads(k, P,length, *((ep_t*) &result), wsize);
    ep_norm(*((ep_t*) &result), *((ep_t*) &result));
...
template<typename T, typename FieldT, multi_exp_method Method,
    typename std::enable_if<(Method == multi_exp_method_pair_avx_threads), int>::type = 0>
T multi_exp_inner(
    typename std::vector<T>::const_iterator bases,
    typename std::vector<T>::const_iterator bases_end,
    typename std::vector<FieldT>::const_iterator exponents,
    typename std::vector<FieldT>::const_iterator exponents_end)
{
    ...
    pair_ifma_threads(k, P, length, *((pair*) &result), wsize);
...
struct MyHash {
    std::size_t operator()(const libff::bls12_381_G1& g1) const {
        size_t hash_value = 0;
        for (size_t i = 0; i < 6; i++)
        {
            hash_value ^= std::hash<unsigned long int>()(g1.X.mont_repr.data[i]);
        }
        // return std::hash<unsigned long int>()(g1.X[0]);
        return hash_value;
    }
};
...
template<typename T, typename FieldT, multi_exp_method Method>
T multi_exp_with_mixed_addition_rmdupli(typename std::vector<T>::const_iterator vec_start,
                                typename std::vector<T>::const_iterator vec_end,
                                typename std::vector<FieldT>::const_iterator scalar_start,
                                typename std::vector<FieldT>::const_iterator scalar_end,
                                const size_t chunks)
{
    ...
    // hashmap
    std::unordered_map<T, FieldT, MyHash> value_scalar_map;

    for (auto value_it = vec_start; value_it != vec_end; ++value_it, ++scalar_start)
    {
        if (value_scalar_map.find(*value_it) != value_scalar_map.end())
        {
            value_scalar_map[*value_it] += *scalar_start;
        }
        else
        {
            value_scalar_map[*value_it] = *scalar_start;
    ...
    for (const auto& entry : value_scalar_map)
    {
        ...
        else if(entry.second == one)
        {
#ifdef USE_MIXED_ADDITION
            acc = acc.mixed_add(entry.first);
#else
            acc = acc + entry.first;
#endif
            ++num_add;
        ...
        {
            if(entry.first != T::zero())
            {
                p.emplace_back(entry.second);
                g.emplace_back(entry.first);
                ++num_other;
            }
            // p.push_back(entry.second);
            // g.push_back(entry.first);
            // ++num_other;
        }
    }
    print_indent(); printf("* Elements of w skipped: %zu (%0.2f%%)\n", num_skip, 100.*num_skip/(num_skip+num_add+num_other));
